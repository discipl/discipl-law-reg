#!/bin/bash
cat "$1"  | jq -r '.acts | [ "act", "actor", "action", "object", "recipient", "preconditions", "create", "terminate", "version", "reference", "juriconnect", "sourcetext", "explanation" ] as $cols | map(. as $row | $cols | map($row[.])) as $rows | $cols, $rows[] | @csv' > acts.csv
